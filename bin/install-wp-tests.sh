...
install_db() {
	...
	# create database
	if [ $(mysql --user="$DB_USER" --password="$DB_PASS"$EXTRA --skip-column-names -e "SHOW DATABASES LIKE '$DB_NAME'") ]
	then
		echo "Recreating will drop the existing test database ($DB_NAME)"
