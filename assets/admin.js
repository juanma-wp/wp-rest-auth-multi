jQuery(document).ready(function($) {
    ...
    $('#add_oauth2_client').on('click', function() {
        ...
        $.post(wpRestAuthMulti.ajaxUrl, data)
            .done(function(response) {
                if (response.success) {
                    showNotice('OAuth2 client added successfully!', 'success');
                    // Clear form
                    $('#new_client_name, #new_client_id, #new_client_redirect_uris').val('');
                    // Reload page to show new client
                    location.reload();
                } else {
                    showNotice(response.data || 'Failed to add OAuth2 client.', 'error');
                }
            })
            .fail(function() {
    ...
    $(document).on('click', '.delete-client', function() {
        ...
        if (!confirm('Are you sure you want to delete this OAuth2 client? This action cannot be undone.')) {
            return;
        }
            ...
            .done(function(response) {
                if (response.success) {
                    ...
                    row.fadeOut(300, function() {
                        ...
                        if ($('.oauth2-existing-clients tbody tr').length === 0) {
                            $('.oauth2-existing-clients').html('<h3>Existing OAuth2 Clients</h3><p>No OAuth2 clients configured yet.</p>');
                        }
                    });
                } else {
                    showNotice(response.data || 'Failed to delete OAuth2 client.', 'error');
                }
            })
            .fail(function() {
                showNotice('Network error occurred.', 'error');
    ...
    $('form').on('submit', function() {
        ...
        if (secretKey && secretKey.length < 32) {
            showNotice('JWT Secret Key must be at least 32 characters long.', 'error');
            return false;
        }
    ...
    $('#new_client_redirect_uris').on('blur', function() {
        ...
        if (invalidUris.length > 0) {
            showNotice('Invalid redirect URIs detected: ' + invalidUris.join(', '), 'warning');
        }
    });
    ...
    // Save draft on input
    $('#new_client_name, #new_client_id, #new_client_redirect_uris').on('input', function() {
        const data = {
